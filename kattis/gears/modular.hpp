...
namespace popup {
     ...
     */
    template<typename T>
    std::optional<std::pair<T,T>> general_chinese_rest_theorem(T a1, T m1, T a2, T m2) {
        auto [gcd, n1, n2] = extended_euclidean<T, __int128>(m1, m2);
        if(((a1 + gcd) % gcd) == (a2 + gcd) % gcd) { // unique solution
            T lcm = (m1*m2)/gcd;
            T ans = ((a1 * (n2 + m1)) % m1) * m2 + ((a2 * (n1 + m2)) % m2) * m1;
            return std::make_pair((ans/gcd) % lcm, lcm);
        }
        return std::nullopt;
    }
