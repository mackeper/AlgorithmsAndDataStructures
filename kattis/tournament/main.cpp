...
int main() {
	...
	while(cin >> glads && glads != 0) {
		...
		popup::FlowGraph<int, double> g(glads*2 + 4);
		int s = glads*2;
		int t = glads*2+1;
		int s2 = glads*2+2;
		int t2 = glads*2+3;
		g.add_edge(s2, s, joining_glads/2, 0);
		g.add_edge(t, t2, joining_glads/2, 0);
		rep(x, 0, glads) {
			g.add_edge(s, x, 1, 0);
			g.add_edge(x+glads, t, 1, 0);
			rep(y, glads+x+1, glads*2) {
                double cost = (pow(glad_stats[x]-glad_stats[y-glads], 2));
				g.add_edge(x, y, 1, cost);
			}
		}
		...
		rep(i, 0, glads) {
			for(auto &edge : g.list_[i]) {
				if(edge->residual() == 0 && (int)edge->to() != s &&
						(int)edge->to() != t && !matched[i] && !matched[edge->to()]) {
					matched[i] = true;
