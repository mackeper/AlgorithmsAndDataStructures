...
public:
  ...
  virtual void traverse_neighbors(size_t node,
                                  std::function<void(size_t, const T &)> f) = 0;
  ...
  // Dijkstra
  std::optional<std::pair<std::vector<size_t>, T>> dijkstra(size_t from,
                                                            size_t to) {
    const auto cmp = [](const std::pair<size_t, T> &a,
    ...
    std::vector<size_t> came_from(capacity_,
...
template <class T> class ListGraph : public Graph<T> {
...
public:
  ListGraph(size_t capacity) {
    ...
    Graph<T>::size_ = 0;
    list_ = std::vector<std::vector<Edge<T>>>(Graph<T>::capacity_,
                                              std::vector<Edge<T>>());
  }
  ...
  virtual void traverse_neighbors(size_t from,
