...
fn solve<R: BufRead>(stdin: &mut R, buffer: &mut String) -> String {
    let mut values: Vec<i64> = get_line_of_integers(stdin, buffer);
    values.sort();
...
mod tests {
    ...
    #[test]
    fn test_1() {
        let mut stdin = "3 4".as_bytes();
        let mut buffer = String::with_capacity(1024);
        assert_eq!(solve(&mut stdin, &mut buffer), "3 4");
    }

    #[test]
    fn test_2() {
        let mut stdin = "987 23".as_bytes();
        let mut buffer = String::with_capacity(1024);
        assert_eq!(solve(&mut stdin, &mut buffer), "23 987");
    }
}
